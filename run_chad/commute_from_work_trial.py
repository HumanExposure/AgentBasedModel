...
Human Activity Database (CHAD) for the **commute from work** activity.

This module contains class :class:`commute_from_work_trial.Commute_From_Work_Trial`.
"""
...
# ===========================================
# class Commute_From_Work_Trial
# ===========================================

class Commute_From_Work_Trial(trial.Trial):
    ...
    def __init__(self, parameters, sampling_params, demographic):
        ...
        self.id = trial.COMMUTE_FROM_WORK

        # the filename for the chad
        self.fname = chad.FNAME_COMMUTE_FROM_WORK
    ...
    def adjust_params(self, commute_dt_mean, commute_dt_std, work_start_mean, work_start_std, \
        ...
        """
        This function adjusts the values for the mean and standard deviation of both commute from work \
        ...
        # set the work parameters for end time (mean end time, standard deviation of end time)
        self.params.work_end_mean   = mg.hours_to_minutes(work_end_mean)
        self.params.work_end_std    = mg.hours_to_minutes(work_end_std)

        # set the commute form work information (mean duration and standard deviation of duration)
        self.params.commute_from_work_dt_mean   = mg.hours_to_minutes(commute_dt_mean)
        self.params.commute_from_work_dt_std    = mg.hours_to_minutes(commute_dt_std)
    ...
    def create_universe(self):
        ...
        # create the universe
        u = super(Commute_From_Work_Trial, self).create_universe()
    ...
    def initialize(self):
        ...
        # the activities
        keys = [mg.KEY_COMMUTE_FROM_WORK, mg.KEY_WORK]

        # obtain the CHAD parameters relevant to commuting from work for each person in the household
        y = super(Commute_From_Work_Trial, self).initialize(keys)
        ...
        commute_start_mean, commute_start_std, commute_end_mean, commute_end_std, commute_dt_mean, commute_dt_std \
            = y[mg.KEY_COMMUTE_FROM_WORK]

        # set the standard deviations to zero (in hours)
        commute_dt_std[:]   = 0
        work_start_std[:]   = 0
        work_end_std[:]     = 0

        # adjust the parameters for the simulation
        self.adjust_params(commute_dt_mean, commute_dt_std, work_start_mean, work_start_std, \
                           work_end_mean, work_end_std)
